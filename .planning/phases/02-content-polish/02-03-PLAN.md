...
must_haves:
  ...
  artifacts:
    ...
    - path: "vite.config.ts"
      provides: "No devtools plugin in production (or conditional inclusion)"
  key_links:
...
Purpose: Required for BRAND-01 (correct title/meta) and BRAND-03 (no devtools in production). Quick, self-contained changes to __root.tsx and vite.config.ts.
...
<task type="auto">
  ...
  <action>
    ...
    `import.meta.env.DEV` is `true` during `pnpm dev` (Vite dev server) and `false` in `pnpm build` production output. No additional configuration needed — this is standard Vite environment variable behavior.
    ...
    No changes to vite.config.ts are needed for this approach.
  </action>
