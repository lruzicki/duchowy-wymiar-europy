...
<task type="auto">
  ...
  <verify>Run `pnpm tsc --noEmit`. Run `grep -n "Powrót\|Brak zdjęć\|Lokalizacja nie" src/routes/locations.\$slug.tsx` — must return zero results (all strings must be in t() calls). Start dev server. Navigate to http://localhost:3000/locations/dabrowa — page must load (even if images are empty, the layout should render). Back-to-map button should be visible. No console errors.</verify>
  <done>Route /locations/dabrowa loads with location name, city/country badges, date badge, image area (empty or populated), and back-to-map button. All UI strings use t(). TypeScript compiles clean.</done>
</task>
