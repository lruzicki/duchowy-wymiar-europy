...
Output: Refactored ProjectsMap.tsx, new locations.$slug.tsx route, updated locations.ts with getLocationBySlug, updated locale files with map.seeMore and map.noLocations keys.
</objective>
...
<task type="auto">
  ...
  <files>src/lib/locations.ts, src/components/figma/ProjectsMap.tsx, src/locales/pl.json, src/locales/en.json, src/locales/de.json, src/locales/uk.json, src/locales/ru.json, src/locales/ar.json</files>
  <action>
    **src/lib/locations.ts:**
    ...
    **All 6 locale files (pl.json, en.json, de.json, uk.json, ru.json, ar.json):**

    Add `map.seeMore` and `map.noLocations` keys to all 6 locale files:
  ...
  <verify>Run `pnpm tsc --noEmit`. Grep for "unsplash" in all src/ files — must return zero results. Grep for "projectsData" in ProjectsMap.tsx — must return zero results. Run `node -e "JSON.parse(require('fs').readFileSync('src/locales/pl.json','utf8'))"` for each locale to confirm valid JSON. Start dev server and confirm map loads with CMS data (dabrowa pin visible), popup shows name + "See more" link.</verify>
  <done>ProjectsMap.tsx has no hardcoded projectsData array, no Unsplash URLs, no Dialog component. Map shows CMS pins with minimal popup linking to /locations/[slug]. Sidebar cards navigate to location pages. All 6 locale files have map.seeMore and map.noLocations keys and remain valid JSON.</done>
</task>
...
- All 6 locale files are valid JSON with map.seeMore and map.noLocations keys added
