...
# CMS-05 is partially addressed here: .env.example documents the required credentials.
# The actual Vercel wiring of those credentials is deferred to Phase 3 (DEPLOY-03).
# Phase 3 plans must revisit CMS-05 and mark it complete after Vercel env vars are set.
...
Purpose: This is the foundation for CMS-managed location pages (Plan 04). Without the schema, the image carousel and dedicated pages cannot be built. The .env.example partially addresses CMS-05 — the credentials are documented. The Vercel dashboard wiring is deferred to Phase 3 (DEPLOY-03), which must confirm all env vars are set before marking CMS-05 complete.
...
<task type="auto">
  ...
  <action>
    Create `.env.example` at the project root (next to `package.json`). This file documents all environment variables needed for Keystatic to work in GitHub storage mode on Vercel. It partially addresses CMS-05 — the credentials are specified so the deployer knows exactly what to set. The actual Vercel dashboard wiring is a Phase 3 task.
...
- CMS-05 Phase 2 portion satisfied: .env.example + GitHub storage config are in place. Vercel dashboard wiring completes in Phase 3 (DEPLOY-03).
</verification>

<success_criteria>
The Keystatic locations collection schema has been extended with images array + city/country/date, storage mode is GitHub, and the server function reflects the new types. The existing dabrowa entry is migrated. .env.example documents the GitHub OAuth credentials required for Vercel deployment (Vercel dashboard wiring deferred to Phase 3). This unblocks Plan 04 (location pages) and satisfies CMS-01, CMS-04, and the Phase 2 portion of CMS-05.
</success_criteria>
