...
<objective>
Extend the Keystatic `locations` collection schema to support multiple images per entry, add city/country/date fields, and switch storage to GitHub mode. Update the `getLocations` server function to return the new fields.

Purpose: This is the foundation for CMS-managed location pages (Plan 04). Without the schema, the image carousel and dedicated pages cannot be built.
Output: Updated keystatic.config.ts, updated src/lib/locations.ts, updated content/locations/dabrowa.mdoc.
</objective>
...
<success_criteria>
The Keystatic locations collection schema has been extended with images array + city/country/date, storage mode is GitHub, and the server function reflects the new types. The existing dabrowa entry is migrated. This unblocks Plan 04 (location pages).
</success_criteria>
